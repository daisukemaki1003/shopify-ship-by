...
} from "../utils/shipping-rate-normalize";
...
const extractRatesWithDebug = (zones: RawShippingZone[]): ShippingRateEntry[] => {
  debugShippingRates(
    "extractRates zones",
    zones.map((z) => ({
      id: z?.id ?? null,
  ...
  const rates = extractRates(zones);
...
export async function fetchShippingRates(shop: string) {
  ...
  return extractRatesWithDebug(zones);
}
